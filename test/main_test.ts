...
Deno.test("break two messages with an additional mask (api exapmle)", async () => {
    ...
    const goldenDecrypt = JSON.parse(
        await Deno.readTextFile("./generated/golden_decrypt.json"));
    // console.log('[');
    // for(const r of res) {
    //     console.log(r, ',');
    // }
    // console.log(']');
    assertEquals(res, goldenDecrypt);
});